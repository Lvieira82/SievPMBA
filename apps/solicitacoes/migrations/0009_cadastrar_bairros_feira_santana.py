...
def cadastrar_bairros_e_areas(apps, schema_editor):
    ...
    for nome_bairro, nome_unidade_curto in BAIRROS:
        unidade = Unidade.objects.filter(nome__startswith=nome_unidade_curto + "/").first()
        if not unidade:
            unidade = Unidade.objects.filter(sigla__startswith=nome_unidade_curto + "/").first()
        if not unidade:
            continue
...
def remover_bairros_e_areas(apps, schema_editor):
    Municipio = apps.get_model("solicitacoes", "Municipio")
    Bairro = apps.get_model("solicitacoes", "Bairro")
    AreaResponsabilidade = apps.get_model("solicitacoes", "AreaResponsabilidade")
...
class Migration(migrations.Migration):
    dependencies = [
        ("solicitacoes", "0008_seed_unidades_feira_santana"),
    ]

    operations = [
        migrations.RunPython(cadastrar_bairros_e_areas, remover_bairros_e_areas),
    ]
