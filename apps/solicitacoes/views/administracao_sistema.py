...
def _pode_gerenciar(scope, acesso):
    ...
    if scope["desenvolvedor"]:
        return True
    if acesso.funcao != "MEMBRO" or scope["perfil"] != acesso.perfil:
        return False
    if acesso.perfil == "COPPM":
        return True
...
def administracao_sistema(request):
    ...
    return render(request, "administracao_sistema/index.html", {"acessos": qs, "perfis": qs, "scope": scope})
            ...
            try:
                with transaction.atomic():
                    user = User.objects.create_user(
                        username=data["matricula"], email=data["email"], password=senha,
                        first_name=data["nome"], is_active=data["ativo"],
                    )
                    acesso = AcessoInstitucional.objects.create(
                        usuario=user, matricula=data["matricula"], cpf=data["cpf"],
                        telefone=data["telefone"], perfil=data["perfil"], funcao=data["funcao"],
                        cpr=data["cpr"], unidade=data["unidade"], primeiro_acesso=True, ativo=data["ativo"],
                    )
...
def usuario_excluir(request, id):
    ...
    if user.is_superuser:
        messages.error(request, "O desenvolvedor principal não pode ser excluído por esta tela.")
        return redirect("administracao_sistema")
    if request.method != "POST":
