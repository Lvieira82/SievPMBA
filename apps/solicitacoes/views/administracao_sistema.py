...
AMBITOS = [("COPPM", "COPPM"), ("CPR", "CPR"), ("UNIDADE", "Unidade")]
FUNCOES = [("GESTOR", "Gestor"), ("MEMBRO", "Membro")]
PERFIS_ACESSO = [("MEMBRO", "Membro"), ("OPERADOR", "Operador")]
...
class UsuarioSistemaForm(forms.Form):
    ...
    def __init__(self, *args, instance=None, scope=None, **kwargs):
        ...
        self.fields["unidade"].queryset = Unidade.objects.filter(ativo=True).select_related("cpr").order_by("cpr__sigla", "nome")
        ...
        if instance:
            ...
            if acesso:
                tipo = "OPERADOR" if acesso.perfil == "OPERADOR" else "MEMBRO"
                ambito = "UNIDADE" if acesso.perfil == "OPERADOR" and acesso.unidade_id else ("CPR" if acesso.perfil == "OPERADOR" else acesso.perfil)
                self.initial.update({"matricula": acesso.matricula, "nome": instance.get_full_name(), "cpf": acesso.cpf or "", "telefone": acesso.telefone or "", "email": instance.email, "perfil": ambito, "perfil_acesso": tipo, "funcao": acesso.funcao, "cpr": acesso.cpr_id, "unidade": acesso.unidade_id, "ativo": acesso.ativo and instance.is_active})
                if scope and not scope["desenvolvedor"]:
                    self.fields["funcao"].initial = "MEMBRO"
                    self.fields["funcao"].widget = forms.HiddenInput()
        ...
        if self.scope and not self.scope["desenvolvedor"]:
            ...
            if self.scope["perfil"] == "COPPM":
                if ambito not in {"COPPM", "CPR", "UNIDADE"}:
                    ...
                    if not cpr: self.add_error("cpr", "Selecione o CPR.")
                    cleaned["unidade"] = None
                elif ambito == "UNIDADE":
                    if not cpr: self.add_error("cpr", "Selecione o CPR.")
                    if not unidade: self.add_error("unidade", "Selecione a unidade.")
                ...
                if perfil_acesso == "OPERADOR" and ambito != "UNIDADE":
                    self.add_error("perfil_acesso", "Operador deve ser vinculado a uma Unidade.")
            elif self.scope["perfil"] == "CPR":
                cleaned["perfil"] = "CPR"
                cleaned["cpr"] = self.scope["cpr"]
                if perfil_acesso == "OPERADOR":
                    if not unidade: self.add_error("unidade", "Selecione a unidade do operador.")
                    elif unidade.cpr_id != self.scope["cpr"].id: self.add_error("unidade", "A unidade deve pertencer ao seu CPR.")
                else: cleaned["unidade"] = None
            ...
            return cleaned
        if funcao != "GESTOR": self.add_error("funcao", "O administrador deve cadastrar gestores institucionais.")
        if perfil_acesso != "MEMBRO": self.add_error("perfil_acesso", "Gestor institucional deve possuir perfil Membro.")
        if ambito == "COPPM":
            cleaned["cpr"] = None; cleaned["unidade"] = None
        elif ambito == "CPR":
            if not cpr: self.add_error("cpr", "Selecione o CPR do gestor.")
            cleaned["unidade"] = None
        elif ambito == "UNIDADE":
            if not cpr: self.add_error("cpr", "Selecione o CPR da unidade.")
            if not unidade: self.add_error("unidade", "Selecione a unidade do gestor.")
            if cpr and unidade and unidade.cpr_id != cpr.id: self.add_error("unidade", "A unidade selecionada não pertence ao CPR informado.")
        else: self.add_error("perfil", "Selecione o âmbito institucional.")
        return cleaned

def _escopo(request):
    if request.user.is_superuser or request.user.is_staff:
        return {"desenvolvedor": True, "perfil": None, "cpr": None, "unidade": None, "funcao": "GESTOR"}
    acesso = getattr(request.user, "acesso_institucional", None)
    if not acesso or not acesso.ativo or not request.user.is_active: return None
    if acesso.perfil == "CPR" and acesso.cpr:
        return {"desenvolvedor": False, "perfil": "CPR", "cpr": acesso.cpr, "unidade": None, "funcao": acesso.funcao}
    if acesso.perfil == "UNIDADE" and acesso.unidade:
        return {"desenvolvedor": False, "perfil": "UNIDADE", "cpr": acesso.unidade.cpr, "unidade": acesso.unidade, "funcao": acesso.funcao}
    if acesso.perfil == "COPPM":
        return {"desenvolvedor": False, "perfil": "COPPM", "cpr": None, "unidade": None, "funcao": acesso.funcao}
    return None

def _pode_gerenciar(scope, acesso):
    if not scope or not acesso: return False
    if scope["desenvolvedor"]: return not acesso.usuario.is_superuser
    if scope["perfil"] == "COPPM":
        return acesso.perfil in {"COPPM", "CPR", "UNIDADE", "OPERADOR"}
    if scope["perfil"] == "CPR":
        return acesso.cpr_id == scope["cpr"].id and acesso.perfil in {"CPR", "UNIDADE", "OPERADOR"}
...
def _sincronizar_perfil_compat(user, data):
    ...
    perfil.cpr = data["cpr"]; perfil.unidade = data["unidade"]; perfil.ativo = data["ativo"]; perfil.save()

def _perfil_modelo(data):
    if data["funcao"] == "GESTOR": return data["perfil"]
...
def administracao_sistema(request):
    ...
    if not scope:
        messages.error(request, "Você não possui permissão para administrar usuários."); return redirect("painel_gestao")
...
def usuario_novo(request):
    scope = _escopo(request)
    if not scope:
        messages.error(request, "Você não possui permissão para cadastrar usuários."); return redirect("painel_gestao")
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not scope["desenvolvedor"]:
                data["funcao"] = "MEMBRO"
                if scope["perfil"] == "CPR": data["perfil"], data["cpr"] = "CPR", scope["cpr"]
                elif scope["perfil"] == "UNIDADE": data["perfil"], data["cpr"], data["unidade"] = "UNIDADE", scope["cpr"], scope["unidade"]
            data["perfil"] = _perfil_modelo(data); senha = _senha_inicial()
            try:
                with transaction.atomic():
                    ...
                    AcessoInstitucional.objects.create(usuario=user, matricula=data["matricula"], cpf=data["cpf"] or None, telefone=data["telefone"], perfil=data["perfil"], funcao=data["funcao"], cpr=data["cpr"], unidade=data["unidade"], primeiro_acesso=True, ativo=data["ativo"])
                    _sincronizar_perfil_compat(user, data); _enviar_senha_inicial(user, senha)
            except Exception:
                ...
                messages.success(request, "Usuário criado. A senha inicial foi enviada para o e-mail cadastrado."); return redirect("administracao_sistema")
...
def usuario_editar(request, id):
    scope = _escopo(request); user = get_object_or_404(User, pk=id); acesso = getattr(user, "acesso_institucional", None)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user.username=data["matricula"]; user.first_name=data["nome"]; user.email=data["email"]; user.is_active=data["ativo"]; user.save(update_fields=["username","first_name","email","is_active"])
            acesso.matricula=data["matricula"]; acesso.cpf=data["cpf"] or None; acesso.telefone=data["telefone"]; acesso.perfil=data["perfil"]; acesso.funcao=data["funcao"]; acesso.cpr=data["cpr"]; acesso.unidade=data["unidade"]; acesso.ativo=data["ativo"]; acesso.save(); _sincronizar_perfil_compat(user, data)
...
def usuario_senha(request, id):
    scope = _escopo(request); user = get_object_or_404(User, pk=id); acesso = getattr(user, "acesso_institucional", None)
    if not scope or not _pode_gerenciar(scope, acesso): messages.error(request, "Você não pode alterar a senha deste usuário."); return redirect("administracao_sistema")
    if request.method == "POST":
...
def usuario_desativar(request, id):
    scope=_escopo(request); user=get_object_or_404(User, pk=id); acesso=getattr(user,"acesso_institucional",None)
    if not scope or not _pode_gerenciar(scope, acesso): messages.error(request,"Você não pode alterar este cadastro."); return redirect("administracao_sistema")
    acesso.ativo=False; acesso.save(update_fields=["ativo","atualizado_em"]); user.is_active=False; user.save(update_fields=["is_active"]); messages.success(request,"Usuário desativado."); return redirect("administracao_sistema")

@login_required
def usuario_excluir(request, id):
    scope=_escopo(request); user=get_object_or_404(User, pk=id); acesso=getattr(user,"acesso_institucional",None)
    if not scope or not _pode_gerenciar(scope, acesso): messages.error(request,"Você não pode excluir este usuário."); return redirect("administracao_sistema")
    ...
    if user.is_superuser: messages.error(request,"O superusuário não pode ser excluído por esta tela."); return redirect("administracao_sistema")
