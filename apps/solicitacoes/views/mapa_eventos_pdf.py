...
def _dados_relatorio_cumprimento(request):
    ...
    for evento in eventos:
        ...
        if log:
            ...
            if pos >= 0:
                geolocalizacao = texto[pos + len("Coordenadas GPS:"):].strip().rstrip(".")
        dados.append({
...
def _gerar_relatorio_cumprimento_pdf(request):
    ...
    titulo = ParagraphStyle("RTitulo", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=17, leading=20, alignment=TA_CENTER, spaceAfter=3)
    subtitulo = ParagraphStyle("RSub", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=11, leading=14, alignment=TA_CENTER, spaceAfter=8)
    ...
    story += [Paragraph("POLÍCIA MILITAR DA BAHIA", titulo), Paragraph("COMANDO DE OPERAÇÕES POLICIAIS MILITARES", subtitulo), Paragraph("RELATÓRIO DE CUMPRIMENTO DAS OPOs", subtitulo)]
...
def _gerar_relatorio_cumprimento_xls(request):
    ...
    out = __import__("io").BytesIO(); wb.save(out); out.seek(0)
