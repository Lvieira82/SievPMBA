...
class UsuarioFluxoForm(forms.Form):
    matricula=forms.CharField(max_length=30,label="Matrícula");nome=forms.CharField(max_length=150,label="Nome completo");cpf=forms.CharField(max_length=14,label="CPF");telefone=forms.CharField(max_length=25,label="Telefone");email=forms.EmailField(label="E-mail")
    ...
    funcao=forms.ChoiceField(choices=[("GESTOR","Gestor"),("MEMBRO","Membro")],required=False,label="Função")
    cpr=forms.ModelChoiceField(queryset=CPR.objects.none(),required=False,label="CPR");unidade=forms.ModelChoiceField(queryset=Unidade.objects.none(),required=False,label="Unidade");ativo=forms.BooleanField(required=False,initial=True,label="Usuário ativo")
    def __init__(self,*args,scope=None,instance=None,**kwargs):
        super().__init__(*args,**kwargs);self.scope=scope;self.instance=instance;self.fields["cpr"].queryset=CPR.objects.filter(ativo=True).order_by("sigla");self.fields["unidade"].queryset=Unidade.objects.filter(ativo=True).select_related("cpr").order_by("nome")
        if scope and not scope["desenvolvedor"]:
            if scope["perfil"]=="COPPM": choices=[("COPPM","Membro COPPM")];self.fields["cpr"].queryset=CPR.objects.none();self.fields["unidade"].queryset=Unidade.objects.none()
            elif scope["perfil"]=="CPR": choices=[("CPR","Membro CPR"),("OPERADOR","Operador")];self.fields["cpr"].queryset=CPR.objects.filter(pk=scope["cpr"].pk);self.fields["unidade"].queryset=Unidade.objects.filter(cpr=scope["cpr"],ativo=True).order_by("nome")
            else: choices=[("UNIDADE","Membro de Unidade"),("OPERADOR","Operador")];self.fields["cpr"].queryset=CPR.objects.filter(pk=scope["cpr"].pk);self.fields["unidade"].queryset=Unidade.objects.filter(pk=scope["unidade"].pk)
            if scope["funcao"]=="MEMBRO": choices=[("OPERADOR","Operador")]
            self.fields["perfil"].choices=choices;self.fields["funcao"].widget=forms.HiddenInput();self.fields["funcao"].initial="MEMBRO"
        if instance:
    ...
    def clean_matricula(self):
        v=self.cleaned_data["matricula"].strip();q=AcessoInstitucional.objects.filter(matricula__iexact=v)
        if self.instance:q=q.exclude(usuario=self.instance)
        if q.exists():raise forms.ValidationError("Esta matrícula já está cadastrada.")
        return v
    def clean_cpf(self):
        v=self.cleaned_data["cpf"].strip();q=AcessoInstitucional.objects.filter(cpf=v)
        if self.instance:q=q.exclude(usuario=self.instance)
        if v and q.exists():raise forms.ValidationError("Este CPF já está cadastrado.")
        return v
    def clean(self):
        ...
        if self.scope and not self.scope["desenvolvedor"]:
            if self.scope["funcao"]=="MEMBRO" and p!="OPERADOR":self.add_error("perfil","Membro só pode cadastrar operadores.")
...
def pode_gerenciar(s,a):
    if not s or not a:return False
    if s["desenvolvedor"]:return True
    if s["funcao"]=="GESTOR":
        if s["perfil"]=="COPPM":return a.perfil=="COPPM" and a.funcao=="MEMBRO"
        if s["perfil"]=="CPR":return a.perfil in {"CPR","OPERADOR"} and a.cpr_id==s["cpr"].id and a.funcao=="MEMBRO"
        if s["perfil"]=="UNIDADE":return a.perfil in {"UNIDADE","OPERADOR"} and a.unidade_id==s["unidade"].id and a.funcao=="MEMBRO"
    if s["funcao"]=="MEMBRO":return a.perfil=="OPERADOR" and ((s["perfil"]=="CPR" and a.cpr_id==s["cpr"].id) or (s["perfil"]=="UNIDADE" and a.unidade_id==s["unidade"].id))
    return False

def _senha():return "".join(secrets.choice(string.ascii_letters+string.digits) for _ in range(12))
def _compat(user,a):PerfilUsuario.objects.update_or_create(usuario=user,defaults={"perfil":a.perfil if a.perfil!="OPERADOR" else "UNIDADE","cpr":a.cpr,"unidade":a.unidade,"ativo":a.ativo})
...
def administracao_sistema(request):
    ...
    if not s["desenvolvedor"]:
        ...
        else:qs=qs.filter(perfil__in=["UNIDADE","OPERADOR"],unidade=s["unidade"])
...
def usuario_editar(request,id):
    ...
    if request.method=="POST":
        ...
        if f.is_valid():
            d=f.cleaned_data;user.first_name=d["nome"];user.email=d["email"];user.is_active=d["ativo"];user.save();a.matricula=d["matricula"];a.cpf=d["cpf"] or None;a.telefone=d["telefone"];a.ativo=d["ativo"];a.save();_compat(user,a);messages.success(request,"Cadastro atualizado.");return redirect("administracao_sistema")
...
def usuario_senha(request,id):
    if not (request.user.is_superuser or request.user.is_staff):messages.error(request,"Somente o administrador pode redefinir senhas.");return redirect("administracao_sistema")
    ...
    if request.method=="POST":
        senha=_senha();user.set_password(senha);user.save(update_fields=["password"]);a.primeiro_acesso=True;a.save(update_fields=["primeiro_acesso","atualizado_em"]);send_mail("Nova senha provisória - SiEv",f"Matrícula: {a.matricula}\nSenha inicial: {senha}",None,[user.email],fail_silently=True);messages.success(request,"Nova senha provisória processada.");return redirect("administracao_sistema")
...
def transferir_solicitacao_segura(request,id):
    ...
    if not s or not pode_ver_solicitacao(request.user,sol) or s["funcao"] not in {"GESTOR","MEMBRO"}:messages.error(request,"Você não possui permissão para transferir solicitações.");return redirect("aprovacoes")
    unidades=Unidade.objects.filter(ativo=True).exclude(pk=sol.unidade_id)
    if s["perfil"] in {"CPR","UNIDADE"}:unidades=unidades.filter(cpr=s["cpr"])
    if request.method=="POST":
        dest=get_object_or_404(unidades,pk=request.POST.get("unidade_destino"));motivo=request.POST.get("motivo","").strip()
        with transaction.atomic():
            TransferenciaSolicitacao.objects.create(solicitacao=sol,unidade_origem=sol.unidade,unidade_destino=dest,usuario=request.user,motivo=motivo);sol.unidade=dest;sol.origem="TRANSFERIDA";sol.save(update_fields=["unidade","origem","atualizado_em"])
