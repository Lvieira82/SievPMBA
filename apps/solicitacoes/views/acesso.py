...
def _dispositivo_autorizado(request, usuario):
    ...
    dispositivo = DispositivoAutorizado.objects.filter(
        usuario=usuario, token_hash=_hash_token(token), ativo=True
    ).first()
    if dispositivo:
...
def _enviar_codigo_novo_navegador(request, usuario):
    ...
    send_mail(
        "Código de acesso ao SiEv",
        (
            f"Olá, {nome}.\n\nFoi identificado um novo navegador tentando acessar o SiEv.\n\n"
...
def verificar_novo_navegador(request):
    ...
    if request.method == "POST":
        ...
        resposta.set_cookie(
            COOKIE_DISPOSITIVO, token, max_age=EXPIRACAO_DISPOSITIVO,
...
def esqueci_senha(request):
    if request.method == "POST":
        ...
        acesso = AcessoInstitucional.objects.select_related("usuario").filter(
            Q(matricula__iexact=identificador) | Q(usuario__email__iexact=identificador)
        ).first()
        if acesso and acesso.ativo and acesso.usuario.is_active and acesso.usuario.email:
            ...
            send_mail(
                "Redefinição de senha - SiEv",
                f"Olá, {acesso.usuario.get_full_name() or acesso.matricula}.\n\nUse o link abaixo para criar uma nova senha:\n{url}",
                None, [acesso.usuario.email], fail_silently=True,
            )
