...
def areas_responsabilidade(request):
    if request.method == "POST":
        ...
        if not bairro_nome:
            ...
            messages.success(
                request,
                f"{municipio.nome} / {bairro.nome} → {unidade.nome} salvo com sucesso.",
            )
    ...
    try:
        ...
        for numero, linha in enumerate(leitor, start=2):
            ...
            municipio = Municipio.objects.filter(
                nome__iexact=dados.get("municipio", ""), ativo=True
            ).first()
            unidade = (
...
def bairros_por_municipio(request, municipio_id):
    municipio = get_object_or_404(Municipio, pk=municipio_id, ativo=True)
    bairros = Bairro.objects.filter(municipio=municipio).order_by("nome")
    unidades_ids = (
        AreaResponsabilidade.objects
