...
from apps.solicitacoes.models_acesso import AcessoInstitucional, CodigoNovoNavegador, DispositivoAutorizado
...
def _enviar_codigo_novo_navegador(request, usuario):
    ...
    send_mail(
        ...
        f"Olá, {nome}.\n\nFoi identificado um novo dispositivo tentando acessar o SiEvPM.\n\nSeu código de confirmação é: {codigo}\n\nO código é válido por {EXPIRACAO_CODIGO_MINUTOS} minutos e pode ser usado uma única vez.\n\nSe você não reconhece este acesso, não informe o código e procure o administrador do sistema.",
        None,
        [usuario.email],
        fail_silently=False,
    )
...
def verificar_novo_navegador(request):
    ...
    if not acesso or not acesso.ativo or codigo.usado or codigo.expira_em < timezone.now() or codigo.tentativas >= MAX_TENTATIVAS_CODIGO:
        messages.error(request, "O código expirou. Faça o login novamente para receber outro código.")
        for chave in ("siev_usuario_pendente", "siev_codigo_pendente", "siev_next"):
...
def esqueci_senha(request):
    if request.method == "POST":
        ...
        acesso = AcessoInstitucional.objects.select_related("usuario").filter(Q(matricula__iexact=identificador) | Q(usuario__email__iexact=identificador)).first()
        if acesso and acesso.ativo and acesso.usuario.is_active and acesso.usuario.email:
            ...
            uid = urlsafe_base64_encode(force_bytes(acesso.usuario.pk))
            url = request.build_absolute_uri(reverse("redefinir_senha", kwargs={"uidb64": uid, "token": token}))
            send_mail("Redefinição de senha - SiEvPM", f"Olá, {acesso.usuario.get_full_name() or acesso.matricula}.\n\nUse o link abaixo para criar uma nova senha:\n{url}", None, [acesso.usuario.email], fail_silently=True)
