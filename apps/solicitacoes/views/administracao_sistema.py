...
class UsuarioSistemaForm(forms.Form):
    ...
    def __init__(self, *args, instance=None, scope=None, **kwargs):
        ...
        if scope and not scope["desenvolvedor"]:
            ...
            # Gestor COPPM: só cadastra Membro COPPM.
            if scope["perfil"] == "COPPM":
                ...
                self.fields["cpr"].initial = None
                self.fields["unidade"].initial = None
                self.fields["cpr"].disabled = True
                self.fields["unidade"].disabled = True

            # Gestor CPR: só cadastra Membro no próprio CPR.
            ...
            # Gestor de Unidade: só cadastra Membro ou Operador na própria Unidade.
        ...
        if instance:
            ...
            if acesso:
                ...
                ambito = "UNIDADE" if acesso.perfil == "OPERADOR" and acesso.unidade_id else acesso.perfil
                self.initial.update({
            ...
            # Ao editar, um gestor continua limitado ao próprio âmbito.
            if scope and not scope["desenvolvedor"]:
                self.fields["funcao"].initial = "MEMBRO"
                self.fields["funcao"].widget = forms.HiddenInput()
                if scope["perfil"] in {"COPPM", "CPR"}:
                    self.fields["perfil_acesso"].choices = [("MEMBRO", "Membro")]
                    self.fields["perfil_acesso"].disabled = True
                    self.fields["perfil_acesso"].initial = "MEMBRO"
    ...
    def clean(self):
        cleaned = super().clean()
        if not self.scope or self.scope["desenvolvedor"]:
        ...
        # Nunca aceitar do POST um âmbito/direcionamento diferente do escopo real.
        perfil_gestor = self.scope["perfil"]
        perfil_acesso = cleaned.get("perfil_acesso")
        cleaned["funcao"] = "MEMBRO"
        cleaned["perfil"] = perfil_gestor

        if perfil_gestor == "COPPM":
            cleaned["perfil_acesso"] = "MEMBRO"
            cleaned["cpr"] = None
            cleaned["unidade"] = None

        elif perfil_gestor == "CPR":
            cleaned["perfil_acesso"] = "MEMBRO"
            cleaned["cpr"] = self.scope["cpr"]
            cleaned["unidade"] = None

        elif perfil_gestor == "UNIDADE":
...
def _pode_gerenciar(scope, acesso):
    ...
    if scope["perfil"] == "CPR":
        return acesso.cpr_id == scope["cpr"].id and acesso.perfil == "CPR"
...
def administracao_sistema(request):
    ...
    if not scope["desenvolvedor"]:
        if scope["perfil"] == "COPPM":
            qs = qs.filter(perfil="COPPM")
        elif scope["perfil"] == "CPR":
            qs = qs.filter(perfil="CPR", cpr=scope["cpr"])
        elif scope["perfil"] == "UNIDADE":
            qs = qs.filter(unidade=scope["unidade"], perfil__in=["UNIDADE", "OPERADOR"])
