...
def detalhe_opo_seguro(request, id):
    ...
    pode_editar_remover = _pode_editar_remover_opo(request, s) and not CumprimentoOPO.objects.filter(opo__solicitacao=s, respondido_em__isnull=False).exists()
    bloqueado_por_apoio = s.apoios.exists()
    return render(request, "gestao/detalhe_opo.html", {"solicitacao": s, "anexos": anexos, "documentos": documentos, "pode_apoio": pode_apoio, "pode_editar_remover": pode_editar_remover, "bloqueado_por_apoio": bloqueado_por_apoio})
