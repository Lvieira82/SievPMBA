...
from .models import Bairro, DocumentoSolicitacao, Municipio, Solicitacao, TipoDocumento, Unidade
from .pdf_security import validar_pdf_upload
from .territorio import bairros_do_municipio, lista_bairros as lista_bairros_api, municipio_tem_multiplas_unidades, unidades_do_municipio, validar_direcionamento
...
def listar_unidades(request, cpr_id):
    ...
    return JsonResponse([{"id": unidade.id, "nome": unidade.nome} for unidade in unidades], safe=False)


def lista_municipios(request):
    termo = request.GET.get("q", "").strip()
    municipios = Municipio.objects.filter(nome__icontains=termo, ativo=True).order_by("nome")[:20]
    dados = []
    for municipio in municipios:
        unidades = unidades_do_municipio(municipio)
        dados.append({"id": municipio.id, "nome": municipio.nome, "multiplas_unidades": unidades.count() > 1})
...
def _configurar_bairro_form(form, municipio):
    ...
    form.fields["bairro"].widget.attrs.update({"class": "form-select", "data-territorial": "true"})
    return multiplas
...
def nova_solicitacao(request):
    ...
    if request.method == "POST":
        ...
        if bairro_id and Bairro.objects.filter(id=bairro_id, municipio=municipio, ativo=True).exists():
...
def _salvar_documentos(request, solicitacao):
    ...
    # O ofício possui input próprio no formulário público.
    ...
    for indice, arquivo in enumerate(arquivos):
        ...
        if not tipo:
            messages.error(request, f"Tipo de documento não cadastrado: {tipo_nome or 'não informado'}.")
            continue
...
def consultar_protocolo(request):
    ...
    erro = None
    if protocolo:
        solicitacao = Solicitacao.objects.select_related("municipio", "unidade", "bairro", "tipo_evento").prefetch_related("documentos__tipo_documento").filter(protocolo=protocolo).first()
        if not solicitacao:
            erro = "Protocolo não encontrado."
    eventos_hoje = Solicitacao.objects.filter(data_evento=timezone.localdate(), status="APROVADA").select_related("municipio", "unidade", "bairro").order_by("hora_inicio", "nome_evento")
    return render(request, "solicitacoes/consultar.html", {"solicitacao": solicitacao, "erro": erro, "eventos_hoje": eventos_hoje})


def corrigir_solicitacao(request, protocolo):
    solicitacao = get_object_or_404(
        Solicitacao.objects.select_related("municipio", "unidade", "bairro", "tipo_evento").prefetch_related("documentos__tipo_documento"),
        protocolo=protocolo,
    )
...
"""
    ...
    return render(request, "solicitacoes/corrigir.html", {"form": form, "solicitacao": solicitacao, "documentos": solicitacao.documentos.all()})


@login_required
def agenda_gestao(request):
    hoje = timezone.localdate()
    eventos = Solicitacao.objects.filter(status__in=["APROVADA", "CORRECAO"], data_evento__lt=hoje).select_related("municipio", "unidade", "bairro").order_by("-data_evento", "-hora_inicio")
    dia = request.GET.get("dia")
    if dia:
        try: eventos = eventos.filter(data_evento=date.fromisoformat(dia))
        except ValueError: dia = ""
    mes = request.GET.get("mes")
    if mes:
        try: eventos = eventos.filter(data_evento__month=int(mes))
        except (TypeError, ValueError): mes = ""
    ano = request.GET.get("ano")
    if ano:
        try: eventos = eventos.filter(data_evento__year=int(ano))
        except (TypeError, ValueError): ano = ""
    anos = [item.year for item in Solicitacao.objects.filter(status__in=["APROVADA", "CORRECAO"], data_evento__lt=hoje).dates("data_evento", "year", order="DESC")]
    return render(request, "gestao/agenda.html", {"eventos": eventos, "anos": anos, "filtro_dia": dia, "filtro_mes": mes, "filtro_ano": ano})
