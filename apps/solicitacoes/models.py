...
class CPR(models.Model):
    coppm = models.ForeignKey(COPPM, on_delete=models.PROTECT, related_name="cprs")
...
class Unidade(models.Model):
    TIPOS = [("BPM", "BPM"), ("CIPM", "CIPM"), ("CIPE", "CIPE"), ("CPR", "CPR"), ("ESPECIALIZADA", "Especializada"), ("OUTRA", "Outra")]
    cpr = models.ForeignKey(CPR, on_delete=models.PROTECT, related_name="unidades")
    nome = models.CharField(max_length=150)
...
class TipoDocumento(models.Model):
    nome = models.CharField(max_length=120, unique=True)
    descricao = models.TextField(blank=True)
    extensoes_permitidas = models.CharField(max_length=100, default="pdf")
    ativo = models.BooleanField(default=True)
    class Meta:
        ordering = ["nome"]
    def __str__(self):
...
class ConfiguracaoUnidade(models.Model):
    unidade = models.ForeignKey(Unidade, on_delete=models.CASCADE, related_name="configuracoes")
...
class CampoPersonalizado(models.Model):
    TIPOS = [("texto", "Texto"), ("numero", "Número"), ("data", "Data"), ("hora", "Hora"), ("boolean", "Sim / Não"), ("lista", "Lista"), ("textarea", "Texto Longo")]
...
class OpcaoCampo(models.Model):
    campo = models.ForeignKey(CampoPersonalizado, on_delete=models.CASCADE, related_name="opcoes")
...
class ValorCampo(models.Model):
    solicitacao_modulo = models.ForeignKey(SolicitacaoModulo, on_delete=models.CASCADE, related_name="valores")
...
class Solicitacao(models.Model):
    STATUS = [("RASCUNHO", "Rascunho"), ("ENVIADA", "Enviada"), ("EM_ANALISE", "Em análise"), ("PENDENTE", "Pendente"), ("CORRECAO", "Correção"), ("APROVADA", "Aprovada"), ("REJEITADA", "Rejeitada"), ("CONCLUIDA", "Concluída")]
...
class DocumentoSolicitacao(models.Model):
    ...
    arquivo = models.FileField(upload_to=upload_documento, validators=[FileExtensionValidator(allowed_extensions=["pdf"])])
...
class MatriculaAutorizada(models.Model):
    ...
    nome = models.CharField(max_length=150)
    posto = models.CharField(max_length=30, blank=True)
...
class PerfilUsuario(models.Model):
    PERFIS = [("COPPM", "Gestor COPPM"), ("CPR", "Gestor CPR"), ("UNIDADE", "Gestor de Unidade")]
...
class TransferenciaSolicitacao(models.Model):
    ...
    usuario = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name="transferencias_realizadas")
